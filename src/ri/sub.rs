    ...
    ($($SrcTL:ident $SrcTU:ident),+) => {
        $(
            ...
            impl<const START_LHS: $SrcTL, const END_LHS: $SrcTL, const START_RHS: $SrcTL, const END_RHS: $SrcTL>
                Sub<$SrcTU<START_RHS, END_RHS>> for $SrcTU<START_LHS, END_LHS>
            where
                $SrcTU<{ START_LHS.checked_sub(END_RHS).unwrap() }, { END_LHS.checked_sub(START_RHS).unwrap() }>: Sized,
            {
                type Output =
                    $SrcTU<{ START_LHS.checked_sub(END_RHS).unwrap() }, { END_LHS.checked_sub(START_RHS).unwrap() }>;

                fn sub(self, rhs: $SrcTU<START_RHS, END_RHS>) -> Self::Output {
